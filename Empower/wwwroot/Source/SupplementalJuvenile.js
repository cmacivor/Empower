...
const SupplementalJuvenile = (props) => {
    ...
    let schoolStatuses = props.schoolValues;
    ...
    useEffect(() => {
        ...
        document.getElementById("txtIDExpirationDate").value = convertedIDExpirationDate;
            ...
            setSchoolValues(schoolStatuses);
    ...
    function handleSchoolStatusChange(event) {
        ...
        document.getElementById("btnDDLSchoolAttended").value = selectedValue;

        let selectedSchoolStatus = schoolStatuses.filter(function(schoolStatus) {
            return schoolStatus.ID === parseInt(selectedValue);
        });

        document.getElementById("btnDDLSchoolAttended").innerHTML = selectedSchoolStatus[0].Name;
    }
    ...
    function studentStatusSelectHandler(event) {
     ...
     let schoolOptions = [];
        if (schoolValues.length > 0) {
            schoolOptions = schoolValues.map((value) =>
            <a key={value.ID} value={value.ID} description={value.Description} onClick={ handleSchoolStatusChange } className="dropdown-item">{value.Name}</a>
        );
     }
...
export default SupplementalJuvenile;